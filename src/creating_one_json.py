...
if __name__=='__main__':
    ...
    logging.debug('\n')
    logging.debug('Reading unified CSV file to transfor to JSON ')
    ...
    logging.debug(f"Read from disk: {config['unified_merged_file']}")
    logging.debug(f'Shape of Data Frame = {df.shape}')

    data={}
    logging.debug('Transforming each row of DataFrame into a dict to store as JSON ...')
    ...
    if params['save_to_disk']:
        ...
        logging.debug(f"JSON stored to disk: {config['json_file']}")