...
if __name__=='__main__':
    ...
    logging = logger.init_logger(config['creating_held_out_log'])
    logging.debug('\n')
    logging.debug('Starting script to split one JSON file with all data into train/test, heldout and unused.')
    ...
    data = json.load(f)
    logging.debug(f'Number of entries found: {len(data):,}')
    ...
    logging.debug('Transforming JSON entries into Admission DataClass, readmission target no computed.')
    ...
    logging.debug(f'Computing average length of stay: {np.average(length_of_stays):.4f}')
    logging.debug(f'Computing std length of stay:     {np.std(length_of_stays):.4f}')
    ...
    logging.debug(f'Begining:                              {begining_dataset}')
    logging.debug(f'Start held-out:                        {start_heldout}')
    logging.debug(f'End held-out:                          {end_heldout}')
    logging.debug(f'End data (usable 30 days prior):       {latest_date}')
    ...
    logging.debug(f'held out:      {len(held_out_data):7,} entries')
    logging.debug(f'train and dev: {len(train_val_data):7,} entries')
    logging.debug(f'unused:        {len(unused_after_heldout):7,} entries')
