...
def build_intervention_dict(admissions: list[health_data.Admission], logging) -> dict:
    code2description = {}
    total_entries=0
    invalid_entries=0
    ...
    logging.debug(f'Duplicated or problematic entries (interventions):  {invalid_entries:,}/{total_entries:,}  ({invalid_entries/total_entries:.4%})')
    return code2description

def build_diagnosis_dict(admissions: list[health_data.Admission], logging) -> dict:
    code2description = {}
    total_entries=0
    invalid_entries=0
    for admission in admissions:
        ...
        for code,text in zip(codes,texts):
            ...
            if not code in code2description:
                ...
                if not text in code2description[code]:
                    invalid_entries+=1
                    code2description[code].add(text)
    logging.debug(f'Duplicated or problematic entries (diagnoses):      {invalid_entries:,}/{total_entries:,}  ({invalid_entries/total_entries:.4%})')
    return code2description
...
if __name__ == '__main__':
    ...
    logging = logger.init_logger(config['building_diag_interv_map_log'])
    ...
    logging.debug(f'Running interventions and diagnoses map generation script using {len(all_data):,} entries.')
    ...
    if params['compute_interventions']:    
        logging.debug(f'Computing intervention mapping over {len(all_admissions)} entries.')

        interventions_dict = build_intervention_dict(all_admissions, logging)

        if params['save_to_disk']:
            with open(config['intervention_dict'], 'w', encoding='utf-8') as f:
                json.dump(interventions_dict, f, default=str)
        else:
            logging.debug('Intervention mapping computed but not stored')
    else:
        logging.debug('Skipping interventions ...')
    # ---------- ---------- ---------- ---------- ---------- ---------- ---------- ---------- 
    # DIAGNOSES
    # ---------- ---------- ---------- ---------- ---------- ---------- ---------- ----------
    if params['compute_diagnoses']: 
        diagnosis_dict = build_diagnosis_dict(all_admissions, logging)

        if params['save_to_disk']:
            with open(config['diagnosis_dict'], 'w', encoding='utf-8') as f:
                json.dump(diagnosis_dict, f, default=str)
        else:
            logging.debug('Diagnosis mapping computed but not stored')
    else:
        logging.debug('Skipping diagnoses ...')