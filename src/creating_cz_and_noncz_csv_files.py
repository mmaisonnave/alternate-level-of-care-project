...
if __name__=='__main__':
    ...
    logging = logger.init_logger(config['creating_database_log'])
    logging.debug('\n')
    logging.debug('Starting with CZ FILES')

    if params['save_to_disk']:
        logging.warning('Results will be stored in risk, changes cannot be undone.')
    else:
        logging.debug('Reults will **NOT** be stored in disk, this is a test run.')
    ...
    logging.debug(f'Found {len(filepaths):,} CZ files to transform, reading and converting to DataFrames ...')
    ...
    logging.debug(f'Number of columns found in all files={column_count}')
    logging.debug(f"Shapes of all DataFrames: {'; '.join([str(df.shape) for df in all_dfs])}")


    for column_ix in range(column_count):
        logging.debug(f'{column_ix:2}: {set([df.columns[column_ix] for df in all_dfs])}')
        assert len(set([df.columns[column_ix] for df in all_dfs]))==1

    df = pd.concat(all_dfs)

    logging.debug(f'Concating all DataFrames, resulting shape= {df.shape[0]:,} x {df.shape[1]}')
    ...
    logging.debug('Changing Institution Number, Patient Age, ALCDays, and Acute Days to int.')
    ...
    logging.debug('Transform Case Weight into a float type (can contain NaN values).')
    new_case_weight=[]
    ...
    if params['save_to_disk']:
        logging.debug(f"Saving to disk. Destination: {config['unified_merged_file_cz']}")
    ...
    # ---------- ---------- ---------- ---------- ---------- ---------- ---------- ----------
    logging.debug('\n')
    logging.debug('Working on Non-CZ filesxs')

    filepaths = [os.path.join(config['data_path'], file) for file in config['noncz_files']]
    assert all([os.path.isfile(filepath) for filepath in filepaths])
    logging.debug(f'Found {len(filepaths):,} Non-CZ files to transform, reading and converting to DataFrames ...')
    ...
    if params['save_to_disk']:
        logging.debug(f"Saving to disk. Destination: {config['unified_merged_file_noncz']}")
